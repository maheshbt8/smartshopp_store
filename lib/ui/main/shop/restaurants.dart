...
import 'package:delivery_owner/ui/main/shop/map.dart';
...
class _RestaurantsScreenState extends State<RestaurantsScreen> {
    ...
    if (_restaurants != null){
      ...
      for (var item in _restaurants){
        ...
        if (item.name.toUpperCase().contains(_searchValue)){
          ...
          list.add(Container(
              height: 50,
              child: buttonsEditOrDelete(item.id.toString(), _editRestaurant, _deleteDialog, windowWidth)));      // edit and delete button
          list.add(SizedBox(height: 20,));
        }
      }
    }
        ...
        Align(
          ...
          child: Container(
              ...
              child: Column(
                children: <Widget>[
                  SizedBox(height: 15,),
                  mapButtonBack(_backButtonPress),
  ...
  _bodyRoot() {
    ...
    list.add(oneItem(strings.get(207), "",
        "${strings.get(162)} ${(_restaurants != null) ? _restaurants.length : 0}",
        _getRandomImageFromRestaurants(), windowWidth, "")); // "Restaurants"  // "Total Count:"
    list.add(SizedBox(height: 5,));
    list.add(buttonsViewAllAndAddNew(() {_changeState("viewRestaurantsList");}, () {
